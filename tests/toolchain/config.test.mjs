...
test('release writes require successful checks and explicit prerelease-only options', () => {
    ...
    const create = steps.findIndex(step => step.run?.includes('gh api --method POST'));
    const upload = steps.findIndex(step => step.uses?.startsWith('softprops/action-gh-release@'));
    assert.ok(create >= 0 && upload > create, 'Atomic creation must precede release Action updates');
    assert.match(steps[create].run, /prerelease: true, make_latest: "false"/);
    assert.equal(steps[upload].with.prerelease, true);
    assert.equal(steps[upload].with.make_latest, 'false');
    assert.equal(steps[upload].with.overwrite_files, false);
