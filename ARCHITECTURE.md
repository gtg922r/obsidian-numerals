...
Package I uses focused local installed testing plus existing reviewed automated checks and exact candidate artifact verification. GitHub-hosted installed testing is deferred optional roadmap work; the retained disabled helper and its 83 NOT_RUN rows are not recovery gates. Node tests alone do not establish Obsidian platform coverage, and unavailable platforms remain explicit gaps.
