...
### Changed
- Make local installed testing plus existing reviewed automated checks the recovery validation path. Defer GitHub-hosted installed testing to the optional roadmap, retain the disabled helper and 83 NOT_RUN rows without making them recovery gates, and preserve exact artifact verification and BRAT-only publication.
- Record the reviewed manual-workflow registration and current-sample helper. At that checkpoint, installed testing awaited workflow approval; the local-validation decision above supersedes that request.
...
### Fixed
- Keep Live Preview inline results above native code-marker hiding after settings and editor refreshes, while retaining selected-source reveal.
